...
cb_str = "Mars"  # P2: size of Earth, but a thinner atmosphere
hc_type = "rising"
N0 = 5000  # average number of unattenuated counts in data
E_kev = 1.5
H = 20000  # km, orbital altitude
bin_size = 1.0
...
class CurveComparison:
    def __init__(self, sat, hc_type, N0, interp_type="cubic"):
        ...
        # Identify a better t0
        self.t0_2, self.t0_guess_list, self.chisq_list = self.locate_t0_step2()
        # Identify resolution-free t0 from the gaussian
        self.dt_e, self.t0_e = self.analyze_chisq()
    ...
    def locate_t0_step2(self):
        ...
        # side_range = 1 / 15 * len(weight_range)
        if cb_str != "Mars":
            t_start_list = np.arange(self.t0_1 - 1, self.t0_1 + 1, desired_precision)
        else:
    ...
    def analyze_chisq(self):
        ...
        t0_e = float(least_squares(chisq_func, x0=self.t0_2).x)
        print("min chisq step 2:" + str(t0_e))
